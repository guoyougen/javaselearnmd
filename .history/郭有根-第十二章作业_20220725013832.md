[//]: # (注释
  ...
  LastEditors: gyg
  LastEditTime: 2022-07-25 01:38:32
  FilePath: \note\郭有根-第十二章作业.md
)
# 任务单作答卡
...
- 在JDK8之前，要求这个局部变量必需是final(原因是内部类也会编译成单独的class文件，没有办法知道外部类方法中的局部变量，所以编译器是把局部变量变为局部内部类自己的一个参数，这些又有参数不一致的问题，导致开发人员)的，JDK8之后，没有了这个限制。
