[//]: # (注释
  ...
  LastEditors: gyg
  LastEditTime: 2022-08-10 15:51:43
  FilePath: \note\markdown\郭有根-第二十四章作业.md
)

# 任务单作答卡

>**开始时间：** 2022-08-10 10:47 **结束时间：** 2022-08-10 11:33
...
![5](https://s2.loli.net/2022/08/10/7LIaMjdEhq3O1xc.png)
...
可以推出简化方法： 前提：推出一个公式，整数m的立方,拆成连续的奇数之和，它的多项式可以推为：m*m+1-m,
如：m=5, 则它的第一项为：5*5+1-5 = 21, 后面的就是23，25，27，29

```java
public class NiKeCheSi {
    public static void YanZheng(int n) {
        int sum = 0;
        if (n <= 0) {
            return;
        }
        ...
        if (sum == n * n * n) {
            System.out.printf("%s == %d == %d^3 尼科彻斯定理成立\n", sb, sum, n);
            return;
        } else {
            System.out.printf("%s == %s != %d^3尼科彻斯定理不成立\n", sb, sum, n);
            return;
        }
    }
...
![6](https://s2.loli.net/2022/08/10/C7MblAK5x4nSrf3.png)